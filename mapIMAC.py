...
import random
import mapLayer
def mapIMAC(nodes,length,hpar,vpar,metal,T,H,L,W,D,eps,rho,weight_var,testnum,data_dir,spice_dir,vdd,vss,tsampling, run, flip_subarray=None):
    ...
    for i in range(len(nodes)-1):
        f.write(f".include 'layer{i+1}run{run}.sp'\n")
    for i in range(len(nodes)-1):
        if i == 2:
            mapLayer.mapLayer(nodes[i],nodes[i+1],i+1,hpar[i],vpar[i],metal,T,H,L,W,D,eps,rho,weight_var,data_dir,spice_dir,run, flip_subarray=flip_subarray)
        else:
            mapLayer.mapLayer(nodes[i],nodes[i+1],i+1,hpar[i],vpar[i],metal,T,H,L,W,D,eps,rho,weight_var,data_dir,spice_dir,run)
